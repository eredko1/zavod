...
if (sb) {
  ...
  await pg.evaluate((t) => window.__game.teleport(t[0], t[1] + 0.05, t[2], 0, 0), sb.top); await pg.waitForTimeout(1200);
  ...
  await pg.evaluate((t) => window.__game.teleport(t[0], t[1] + 0.05, t[2], Math.PI, 0), sb.bottom); await pg.waitForTimeout(1200);
...
await pg.evaluate((d) => { const a = d.inside, o = d.outside; window.__game.teleport(a[0], a[1] + 0.05, a[2], Math.atan2(-(o[0] - a[0]), -(o[2] - a[2])), 0); }, rd); await pg.waitForTimeout(800);
...
for (let i = 0; i < 4; i++) { await pg.keyboard.press('KeyF'); await pg.waitForTimeout(350); }
ok((await pg.evaluate(() => window.__game.hangout.roofDoor().open)), 'four shoves bust it open');
await pg.waitForTimeout(600); await pg.keyboard.down('KeyW'); await pg.waitForTimeout(1500); await pg.keyboard.up('KeyW');
...
// take a leak
await pg.keyboard.press('KeyP'); await pg.waitForTimeout(1600); await pg.screenshot({ path: `${out}/leak.png` });
