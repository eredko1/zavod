...
await A.evaluate(() => { const [x, , z] = window.__game.hangout.state().igor; window.__game.teleport(x + 1.5, 0, z, Math.PI / 2, 0); }); await A.waitForTimeout(900); await A.keyboard.press('KeyF'); await A.waitForTimeout(400); await A.keyboard.press('Digit2'); await A.waitForTimeout(300); await A.keyboard.press('Digit1'); await A.waitForTimeout(300);
