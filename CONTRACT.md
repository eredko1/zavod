...
## Accuracy standard for real-place maps (terminal = Grand Central Terminal, wsp, sbu) and the critic
- Geographic accuracy: build from a measured plan. Pull the real footprint (satellite/OpenStreetMap — WSP ≈ 9.75 acres, arch at the north edge on the 5th Ave axis, fountain plaza centered south of it, radiating paths, chess plaza SW corner, Garibaldi east of the fountain, playgrounds NE/NW, mounds SW; SBU Academic Mall axis from the Melville Library east/west with the SAC, Frey Hall, Staller Center, Wang Center, Javits, Harriman, the fountain and steps at their real relative positions). Convert to metres 1:1 where the map fits (WSP fits ~200×160 m — use it whole; SBU take the Academic Mall core ~250×200 m). Landmark proportions must be right (WSP arch 23.5 m tall, 9.1 m wide opening; fountain basin ~24 m diameter with the sunken plaza and steps).
- Pictorial accuracy: the agent downloads 8–12 reference photos per map into `qa/refs/<id>/` (Wikimedia Commons / Flickr CC, or any public photo — local use only, gitignored) covering each landmark, the paving pattern (WSP hexagonal asphalt pavers and granite; SBU's grey concrete brick and the library's brutalist ribbed concrete), typical vegetation (WSP London planes / elms; SBU pines and oaks), lampposts, benches, fences, facades of the surrounding blocks. Materials, colors and skyline must match those photos.
- The critic scores every map on the same rubric (lighting, materials, composition, clutter density, readability, fps) AND, for real places, a 'recognizability' score: a blind comparison "does frame B (ours) read as the same place as frame A (photo)?" — target ≥ 7/10 before a map is accepted. Maps that fail iterate like everything else.
- `terminal` is the REAL Grand Central Terminal (Manhattan), not an homage: Main Concourse 275×120 ft floor, 125 ft ceiling (≈ 84×37×38 m) with the cerulean celestial ceiling and gold-leaf constellations (reversed zodiac), the three 60-ft arched windows on the east and west walls plus the south-wall clerestory, the information booth with the four-faced opal clock, Tennessee-pink-marble floor and the west (to Vanderbilt Hall / 42nd St) and east (to the balcony) staircases modelled on the Paris Opéra, the north balcony, the Vanderbilt Hall waiting room, the ramps down to the Dining Concourse with the Guastavino-tile Whispering Gallery outside the Oyster Bar, the track level with platforms, and below that the subway mezzanine and 4/5/6 platforms (tile, I-beams, a stopped train). Levels: concourse 0, balconies ≈ +6 m, dining concourse ≈ −6 m, subway ≈ −12 m (bounds and groundHeight accordingly). Research first (plans, dimensions, photos → src/world/terminal/RESEARCH.md), collect 12+ reference photos, generic signage text only, no real advertising.
...
## Vehicles (motorcycles) — 2026-09-20
- Module `src/vehicles.js` (+ `src/vehicles/*`), registered in main and updated BEFORE player. API: `list` (Bike[]), `mounted` (Bike|null), `mount(bike)`, `dismount()`, `qaSpawn(x,z,yaw)`.
- Bike = procedural police/military motorcycle (frame, tank, seat, forks, two wheels with spokes/tyres, headlight, tail light, mirrors, panniers), ~3k tris, dark matte + chrome accents; 3–5 per map placed on roads/spawns via `ctx.world.vehicleSpots` if a map provides it (`[{x,z,yaw}]`), else auto-picked open ground near player spawns.
- Mount: walk within 2 m and press F (`input.consume('KeyF')`; touch: a MOUNT button appears — touch.js exposes nothing yet, so read `ctx.input.pressed.has('KeyF')` and also `ctx.bus.on('ui:mount')`). While mounted set `ctx.player.mounted = bike`; player.update then yields: vehicles.js MUST set `ctx.player.position` (feet at the seat, ~0.9 m up), `ctx.player.yaw` (bike heading + free-look offset from mouse dx, ±60°), `ctx.player.pitch`, and `ctx.camera.position/rotation` each frame (eye ≈ position.y + 0.75, lean into turns ±12°). Dismount with F: place the player 1 m beside the bike, clear `mounted`.
- Arcade physics: throttle W (0→22 m/s over ~4 s), brake/reverse S, Space = hard brake + rear slide, A/D steer (rate falls with speed), lean angle from steering×speed, suspension bob; collide with `ctx.colliders` (AABB slide, speed loss on impact, small bounce), stay on `ctx.world.groundHeight`, block on `ctx.world.bounds`. Engine audio via `ctx.audio.play('bike_idle'|'bike_rev')` if present (guard).
