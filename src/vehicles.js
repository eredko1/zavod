...
const BIKE_SPEC = {
  ...
  chaseD: 4.4, chaseH: 0.32, chaseLook: 1.45, hitMul: 5.5, hitSlow: 0.75, engineBrake: 0.6,
};
const CAR_BASE = {
  car: true, max: 30, revMax: 7, accel: 9, brake: 13, hard: 5.5, hardYaw: 1.3, grip: 13, hardGrip: 1.1, latG: 1.05, steerLow: 0.6,
  band: 0.28, stepUp: 0.36, leanK: -0.2, lookYaw: 110 * DEG, chaseD: 6.8, chaseH: 0.3, chaseLook: 1.6, hitMul: 8.5, hitSlow: 0.9, engineBrake: 1.4,
};
const CAR_SPECS = {};
...
const SURF = { hard: { max: 1, accel: 1, grip: 1, drag: 0, rumble: 0 }, soft: { max: 0.8, accel: 0.85, grip: 0.75, drag: 0.8, rumble: 0.3 }, sand: { max: 0.5, accel: 0.6, grip: 0.4, drag: 3.2, rumble: 1 } };
...
function dismount() {
  ...
  const sx0 = sp.hx + 0.55, sz0 = sp.hz + 0.6;
  ...
  for (const [sx, sz] of cands) {
    ...
    const gy = floorAt(x, z, bike.pos.y, null);
    if (rectBlocked(S.grid, x - 0.36, z - 0.36, x + 0.36, z + 0.36, gy + 0.05, gy + 1.75, null)) continue;
  ...
  bike.vel.set(0, 0, 0); bike.speed = bike.fwdSpeed = 0; bike.throttle = 0; bike.vy = 0; bike.air = false; bike.parked = true; parkBox(bike); rebuildGrids();
...
function applyChase(v, dt) {
  ...
  const lx = tx + fx * 3 - _v.x, ly = ty + 0.35 - _v.y, lz = tz + fz * 3 - _v.z;
...
function updateUI(ctx) {
  ...
  U.cam.classList.toggle('show', playing && !!S.mounted);
